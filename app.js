...
const authenticate = (req, res, next) => {
  ...
  if (!token || !sessions[token]) return res.status(401).json({ success: false, message: "Unauthorized." });
...
app.post("/posts/:id/save", authenticate, (req, res) => {
  const postId = parseInt(req.params.id);
  const { userId } = req.body;
  const posts = readPosts();
  const post = posts.find(post => post.id === postId);
  ...
  if (post.savedBy.includes(userId)) return res.status(400).json({ success: false, message: "Post already saved." });
...
app.post("/users/:username/follow", authenticate, (req, res) => {
...
app.get("/users/:username", (req, res) => {
  ...
  const posts = readPosts().filter(post => post.username === user.username);
  res.json({
    success: true,
    user: {
      ...user,
...
app.get("/users/:userId/saved", authenticate, (req, res) => {
